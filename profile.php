...
$userInfo = $conn->prepare("SELECT name, email, role FROM users WHERE id = :user_id");
$userInfo->execute(['user_id' => $user_id]);
$userData = $userInfo->fetch(PDO::FETCH_ASSOC);
$name = $userData ? htmlspecialchars($userData['name']) : 'Неизвестно';
$email = $userData ? htmlspecialchars($userData['email']) : 'Неизвестно';
$role = $userData ? htmlspecialchars($userData['role']) : 'Неизвестно';

// Дальше идут все те же запросы, что в dashboard.php
// Просто заменяй $user_id = $_SESSION['user_id'] на уже установленную переменную $user_id
// И подключи dashboard.html из которого скопируй весь блок отображения результатов
// или вставь прямо сюда как у тебя уже было
$query = "
    SELECT
        u.id,
        u.name,
...
<main style="margin-top: 50px;">
    ...
    <div class="container">
        ...
        <section class="contacts">
            <h2>Почта</h2>
            <p>Адрес электронной почты: <strong><?= htmlspecialchars($email)?></strong></p>
        </section>
...
<script>
    const text = "Информация о пользователи";
    let index = 0;
